<div class="relative">
    ...
    <div class="max-w-7xl mx-auto p-6 bg-zinc-100 rounded-xl shadow-md">
        <div class="flex justify-between border-b border-gray-200 pb-3 mb-5">
            ...
            <a href="{{ route('repairshop.create') }}" class="flex gap-1 items-center bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
                <flux:icon.plus class="w-5 h-5" /> Nova Oficina
            </a>
        </div>
            ...
            <tbody>
                @forelse($repairShops as $repairshop)
                    <tr class="text-center border-b">
                        <td class="p-2">{{ $repairshop->name }}</td>
                        <td class="p-2">{{ $repairshop->cnpj }}</td>
                        <td class="p-2 text-left">{{ $repairshop->location }}</td>
                        <td class="p-2">
                            @if ($repairshop->is_active)
                        ...
                        <td class="p-2">
                            <a
                                href="{{ route('repairshop.edit', $repairshop->id) }}"
